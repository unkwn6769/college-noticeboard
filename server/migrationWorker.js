import {
  tryServerSideDriveCopy,
  withDriveOperationTimeout,
} from "./migrationDriveCopy.js";
  ...
  const result = await pool.query(
    ...
    SET
      ...
      bytes_transferred = CASE
        WHEN transfer_phase = 'byte_transfer' THEN size_bytes
  ...
  await pool.query(
    ...
    WITH counts AS (
      SELECT
        ...
        COUNT(*) FILTER (WHERE status = 'reconciliation_expired')::bigint AS reconciliation_expired_count,
        COUNT(*) FILTER (WHERE status = 'ambiguous_identity')::bigint AS ambiguous_count,
        COUNT(*) FILTER (WHERE status IN ('pending', 'running', 'reconciling'))::bigint AS nonterminal_count
      FROM google_drive_account_migration_items
      WHERE migration_id = $1
    )
    ...
    SET
      ...
      status = CASE
        WHEN counts.total_count > 0
          ...
          AND counts.failed_count + counts.reconciliation_expired_count + counts.ambiguous_count > 0
          THEN 'failed'
        WHEN counts.total_count > 0
          AND counts.nonterminal_count = 0
      ...
      current_file_id = CASE
        WHEN counts.total_count > 0
          AND counts.nonterminal_count = 0
          AND (counts.completed_count = counts.total_count
            OR counts.failed_count + counts.reconciliation_expired_count + counts.ambiguous_count > 0)
          THEN NULL
...
export async function retryFailedSourceDeletion(
  ...
  // Atomically claim cleanup work so two disposable runners cannot both
  // perform the same source deletion concurrently. The claim is durable for
  // five minutes and is replaced by the normal retry/backoff schedule on
  // failure or cleared on success.
  const cleanupClaim = await pool.query(
    `
      UPDATE google_drive_account_migration_items
      SET cleanup_next_attempt_at = NOW() + INTERVAL '5 minutes',
          updated_at = NOW()
      WHERE id = $1
  ...
  try {
    const targetResponse = await withDriveOperationTimeout(
      (signal) =>
        targetDrive.files.get(
          {
...
async function setItemTransferPhase(itemId, leaseGeneration, transferPhase) {
  const result = await pool.query(
    `
      UPDATE google_drive_account_migration_items
      SET transfer_phase = $1, updated_at = NOW()
      WHERE id = $2 AND lease_generation = $3
    `,
    [transferPhase, itemId, leaseGeneration],
  );
  assertFencedUpdate(result, itemId);
}

/*
 * Google-side files.copy() does not expose wire-byte progress. bytes_transferred
 * therefore only represents observed bytes from a byte-streaming transfer.
 * The production migration path never fabricates this metric.
 */
export async function updateItemProgress(
  ...
  const result = await pool.query(
    ...
    SET
      ...
      END,
      bytes_transferred = CASE
        WHEN transfer_phase = 'byte_transfer' THEN $1
        ELSE bytes_transferred
      END,
      transfer_phase = $2,
      updated_at = NOW()
...
export async function finalizeCancellationIfIdle(migrationId) {
  ...
  try {
    ...
    if (
      migrationResult.rowCount === 0 ||
      !migrationResult.rows[0].cancel_requested
    ) {
      await client.query("ROLLBACK");
      return false;
    }
...
export async function migrateOneItem(
  ...
  try {
    ...
    const sourceMetadataResponse = await withDriveOperationTimeout(
      (signal) =>
    ...
    if (!targetFileId) {
      throw new Error(
        "Server-side Google Drive copy did not produce a target file"
      );
    }
    ...
    if (!targetMetadata?.size && sourceMetadata.size != null) {
      const targetMetadataResponse = await withDriveOperationTimeout(
        (signal) =>
          targetDrive.files.get(
            {
              fileId: targetFileId,
 ...
 */
     ...
     */
    try {
      const finalTargetResponse = await withDriveOperationTimeout(
        (signal) =>
          targetDrive.files.get(
            {
              fileId: targetFileId,
