...
[[ "$FRONTEND_URL" == "https://college-noticeboard-final-343.pages.dev" ]] || fail "FRONTEND_URL must be https://college-noticeboard-final-343.pages.dev"
...
if ! (npx wrangler pages project list --json | python3 -c 'import json,sys; data=json.load(sys.stdin); print(any(p.get("name") == "college-noticeboard-final" for p in data))' | grep -q True); then
  npx wrangler pages project create college-noticeboard-final --production-branch main
fi

echo "== Deploying Pages =="
npx wrangler pages deploy dist --project-name=college-noticeboard-final --branch=main
