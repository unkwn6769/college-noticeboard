...
const allowedOrigins = new Set([
  "http://localhost:5173",
  "https://college-noticeboard-final-343.pages.dev",
  ...configuredOrigins,
]);
    ...
    try {
      await pool.query(
        `
          ...
          SET
            ...
            status = CASE
              WHEN counts.total_count > 0 AND counts.nonterminal_count = 0
      ...
      const result = await pool.query(
        ...
        SELECT
          ...
          item.size_bytes,
          CASE
            WHEN item.transfer_phase = 'byte_transfer'
              THEN GREATEST(
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await logAdminActivity({
        req,
        action: "migration_resume_scheduled",
        entityType: "migration",
        entityId: migrationId,
