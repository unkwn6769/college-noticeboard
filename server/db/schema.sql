...
CREATE TABLE IF NOT EXISTS google_drive_account_migrations (
    ...
    target_account_id TEXT NOT NULL
        REFERENCES google_drive_accounts(id)
        ON DELETE RESTRICT,
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','running','waiting_for_storage','reconciling','reconciliation_expired','completed','failed','cancelled')),
    file_limit INTEGER,
...
CREATE TABLE IF NOT EXISTS google_drive_account_migration_items (
    ...
    transfer_phase TEXT NOT NULL DEFAULT 'pending',
