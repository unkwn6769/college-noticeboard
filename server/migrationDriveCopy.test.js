...
test("403 rejects without a streaming fallback", async () => {
  const { context, calls } = makeContext();
  context.targetDrive.files.copy = async (...args) => {
    calls.copy.push(args);
  ...
  await assert.rejects(() => tryServerSideDriveCopy(context), /Server-side Google Drive copy rejected/i);
  assert.equal(calls.copy.length, 1);
  assert.equal(calls.permissionCreate.length, 0);
});

test("404 attempts Google-side permission recovery", async () => {
  const { context, calls } = makeContext();
  context.targetDrive.files.copy = async (...args) => {
    calls.copy.push(args);
    const error = new Error("not found");
  ...
  await assert.rejects(() => tryServerSideDriveCopy(context), /Server-side Google Drive copy unavailable/i);
  assert.equal(calls.copy.length, 6);
  assert.equal(calls.permissionCreate.length, 1);
  assert.equal(calls.permissionDelete.length, 1);
});

test("share succeeds but copy remains 404, then fails and cleans permission", async () => {
  const { context, calls } = makeContext();
  context.targetDrive.files.copy = async (...args) => {
    calls.copy.push(args);
    const error = new Error("not found");
...
test("disabled server-side copy is a hard error", async () => {
  const { context, calls } = makeContext({ enabled: false });

  await assert.rejects(() => tryServerSideDriveCopy(context), /server-side Google Drive copy is required/i);
...
test("hung Google-side copy times out and enters reconciliation", async () => {
  const { context, calls } = makeContext();
  context.targetDrive.files.copy = async (...args) => {
    calls.copy.push(args);
  ...
  const result = await tryServerSideDriveCopy({
    ...context,
  ...
  assert.equal(calls.copy.length, 1);
  assert.equal(calls.reconcile.length, 1);
  assert.match(calls.reconcile[0][2], /timed out|uncertain/i);
});
